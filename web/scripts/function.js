...
function modifyQuantity(id,index){
	var number = document.getElementById("number_id_" + id);
	location.href = "cart.do?action=mod&id="+id+"&quantity="+number.value+"&index="+index;
}
function removeShopping(id,index) {
	location.href = "cart.do?action=remove&id="+id+"&index="+index;
}